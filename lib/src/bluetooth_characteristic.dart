...
class BluetoothCharacteristic {
  ...
  Future<List<int>> read() async {
    ...
    FlutterBlue.instance._log(LogLevel.info, 'remoteId: ${deviceId.toString()} characteristicUuid: ${uuid.toString()} serviceUuid: ${serviceUuid.toString()}');
    ...
    return FlutterBlue.instance._methodStream
        ...
        .where((p) => (p.remoteId == request.remoteId) && (p.characteristic.uuid == request.characteristicUuid) && (p.characteristic.serviceUuid == request.serviceUuid))
  ...
  Future<Null> write(List<int> value, {bool withoutResponse = false}) async {
    ...
    return FlutterBlue.instance._methodStream
        ...
        .where((p) => (p.request.remoteId == request.remoteId) && (p.request.characteristicUuid == request.characteristicUuid) && (p.request.serviceUuid == request.serviceUuid))
        .first
        .then((w) => w.success)
